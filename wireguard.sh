...
# https://github.com/Nyr/wireguard-install  <<linsongui@gmail.com>>
...
enter_server_address() {
    echo
    use_dns_name=0  # Automatically disable using a DNS name
...
select_port() {
  if [ "$auto" = 0 ]; then
    # Skip asking for user input and directly set port to 51820
    echo "Port is set to default: 51820"
  else
   [ -n "$server_port" ] && port="$server_port" || port=51820
  fi

  # Directly use the chosen or default port
  [[ -z "$port" ]] && port=51820
}
...
enter_first_client_name() {
  if [ "$auto" = 0 ]; then
    # Skip asking for user input and directly set client name to "fei_taiwan"
    unsanitized_client="fei_taiwan"
  else
   if [ -n "$first_client_name" ]; then
     unsanitized_client="$first_client_name"
   else
  ...
  # Call set_client_name to sanitize the name, but don't prompt for input
  set_client_name
}
